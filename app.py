...
# 3. ENGINE - REINFORCED FOR ANCHOR TAGS
async def run_snipe(d, c_slug, target_time):
    async with async_playwright() as p:
        ...
        try:
            ...
            # FIXED SYNTAX: Properly closed the timeout parenthesis
            ...
            # STRIKE LOGIC: Targeting the <a> tag from your Inspect data
            st.warning(f"Searching for {time_str} slot...")
            await pg.evaluate("window.scrollTo(0, document.body.scrollHeight)")
            await asyncio.sleep(2)

            # Targeted Anchor Selector
