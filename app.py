...
# 3. DATE LOGIC (Auto-Strike + Manual Override)
...
if use_auto:
    ...
    target_date = st.sidebar.date_input("Manual Target Date", datetime.date.today() + datetime.timedelta(days=8))
...
with st.sidebar:
    ...
    if st.button("Save Preferences"):
        sb.table("sniper_prefs").upsert({
            "id": 1, "email": user_email, "min_time": str(start_time), "max_time": str(end_time)
        }).execute()
